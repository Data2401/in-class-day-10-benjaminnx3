...
```{r}
in_2010 <- grants[grants$start_year == 2010,]
in_2010
```
...
```{r}
US_PE <- rownames_to_column(US_PE, var = "Category")
US_PE
```
...
```{r}
personal_care <- US_PE[US_PE$Category == "Personal Care",]
pc_in_1940 <- personal_care[personal_care$Year == "X1940",]
sum(pc_in_1940$Expenditures)
```


How much money was spent on Food and Tobacco in 1960?
```{r}
food_tobacco <- US_PE[US_PE$Category == "Food and Tobacco",]
ft_in_1960 <- food_tobacco[food_tobacco$Year == "X1960",]
sum(ft_in_1960$Expenditures)
```


What was the highest expenditure category in 1960?
```{r}
max_expenditures <- US_PE[US_PE$Expenditures == max(US_PE$Expenditures), ]
in_1960_max <- max_expenditures[max_expenditures$Year == "X1960", ]
in_1960_max$Category
```
...
lowest_category <- function(Year) {
  year <- US_PE[US_PE$Year == Year, ]
  year_lowest <- year[year$Expenditures == min(year$Expenditures), ]
  return(year_lowest$Category)
}
...
```{r}
sapply(c("X1940", "X1945", "X1950", "X1955", "X1960"), lowest_category)
```
